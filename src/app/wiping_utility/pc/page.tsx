...
export default function StartPcWorkflow() {
  ...
  async function proceed() {
    ...
    setLoading(true);                         // local scan overlay
    await new Promise((r) => setTimeout(r, 1400)); // simulate capability scan
    router.replace('/wiping_utility/pc/progress'); // open route with its own loader
  }
    ...
    <div className="min-h-screen bg-[#0b0f13] text-[#e6eef8] flex items-center justify-center p-6">
      <div className="w-full max-w-3xl">
        <header className="mb-8 text-center">
          <h1 className="text-3xl font-semibold tracking-tight">Start PC Wipe</h1>
          <p className="mt-2 text-sm text-[#9fb2c8]">
            One‑click mode: the utility will auto‑detect drives and choose the safest standards‑aligned method.
          </p>
        </header>
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 backdrop-blur-[2px]" role="dialog" aria-modal="true">
